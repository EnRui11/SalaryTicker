...
오늘 지금까지 번 금액을 1초마다 갱신해 보여줍니다. Mac 메뉴 막대에서, iPhone에서, Apple Watch에서, 그리고 다이나믹 아일랜드에서.
...
- **계정 없음, 네트워크 없음, 사용 정보 수집 없음.** 모든 계산은 직접 입력한 설정만으로 쓰고 있는 기기 안에서 이루어집니다.
- **화면은 넷, 계산은 하나.** Mac, iPhone, Apple Watch, 다이나믹 아일랜드가 모두 같은 도메인 코드를 읽으므로, 1초가 얼마인지를 두고 서로 어긋날 수 없습니다.
...
이 명령은 릴리스 바이너리를 빌드하고, 소스에서 앱 아이콘을 생성하고, `SalaryTicker.app`을 조립한뒤 애드혹 서명하고, `/Applications`에 복사한 다음 실행합니다. `make app`은 설치만 하지 않을 뿐 같은 일을 합니다.
...
**iOS 26 / watchOS 26**, Xcode 26, 그리고 [xcodegen](https://github.com/yonaskolb/XcodeGen)(`brew install xcodegen`)이 필요합니다.
...
지금으로서는 시뮬레이터 전용입니다. 모든 타깃이 코드 서명을 끈 채로 빌드되기 때문입니다. 실제 기기에 올리려면 먼저 Xcode에 Apple ID를 추가해야 하고, 무료 계정의 프로비저닝 프로파일은 7일이 지나면 만료됩니다. 그다음부터는 다시 빌드하기 전까지 앱이 열리지 않습니다.

실제 iPhone에서는 워치 앱을 따로 설치하지 않습니다. 워치 앱은 폰 앱 **안에** 들어 있으므로, 폰 앱을 설치한 다음 자동 앱 설치를 켜 두거나, iPhone의 **Watch** 앱을 열어 사용 가능한 앱 목록의 SalaryTicker 옆에서 설치를 누르면 됩니다. 시뮬레이터에는 그런 장치가 아예 없고, 그래서 `make watch`가 워치에 직접 올려 줍니다.
...
| **반일** 무급 | 기본급 반일치만큼 손해입니다. Mac에서는 날짜를 오른쪽 클릭하고, iPhone에서는 길게 눌러 오전 또는 오후 휴무를 고르세요. |

오전 휴무와 오후 휴무는 드는 돈이 똑같습니다. 다른 것은 숫자가 언제 움직이느냐입니다. 오후 휴무면 출근부터 반나절만 세고 멈추고, 오전 휴무면 기다렸다가 퇴근까지 셉니다. 그래서 자리에 없는 동안 숫자가 올라가는 일은 없습니다.
...
패널에 나오는 것과 같은 네 가지 숫자를, 같은 순서로 보여줍니다. 둘 다 쓰는 사람이 같은 앱을 두 번 익힐 이유는 없기 때문입니다.

<img src="docs/phone.png" width="300" alt="폰: 오늘 번 금액, 그 금액을 만드는 단가들, 이번 달 누계, 그리고 언제 다 모으게 되는지 날짜가 붙은 목표 하나."> <img src="docs/watch.png" width="300" alt="워치: 오늘 번 금액, 퇴근까지 남은 시간, 이번 달 누계, 그리고 패널에 표시해 둔 첫 번째 목표.">

### 설정 옮기기

Mac에서 **설정 → 시스템 → iPhone으로 보내기**를 열고 폰 카메라를 QR 코드에 비추세요. 급여, 근무 시간, 근무일, 휴가, 목표까지 전부 넘어가므로, 폰은 기본값이 아니라 원래 쓰던 숫자에서 시작합니다.

그 아래에서 코드가 담고 있는 것은 링크입니다. 애초에 가져오기를 테스트할 수 있는 것도 그 덕분입니다. 시뮬레이터에는 카메라가 없지만, URL은 건네줄 수 있기 때문입니다.

거기에는 급여가 담겨 있고, 그래서 복사할 수 있는 문자열이 아니라 그림입니다. 화면에 뜬 코드는 카메라로 들어갈 뿐 다른 곳으로는 가지 않지만, 공유 시트 안에서 텍스트가 되는 순간 원래 갈 일이 없던 곳까지 흘러갈 가능성이 생깁니다.

### 폰

Mac의 탭 대신 스크롤 화면 하나입니다. 폰은 어차피 스크롤하는 물건이고, 탭으로 나누면 바꾸러 들어온 항목이 어느 탭에 있을지 짐작해야 하는 뒤편으로 숨어 버리기 때문입니다.

**목표는 여기, 메인 화면에서 추가합니다.** 시트가 이름과 금액을 먼저 물은 다음에야 무언가를 만듭니다. 이름을 바꾸고, 금액을 고치고, 순서를 바꾸고, 지우는 일은 설정에서 합니다.

### 워치

워치 앱은 자체 설정을 갖지 않고, 설정을 입력할 방법도 주지 않습니다. 시계로는 QR 코드를 찍을 수 없기 때문입니다. 워치 앱은 폰을 기다리고, 폰은 설정이 바뀔 때마다 가장 최신 설정을 보냅니다. 그래서 폰 앱을 적어도 한 번은 열어 본 적이 있어야 하며, 그렇지 않으면 워치에는 보여 줄 것이 없습니다. 워치 페이스용 컴플리케이션도 있습니다.

### 다이나믹 아일랜드와 잠금 화면

**설정 → 표시 → 다이나믹 아일랜드**에서 켜고, 잠금 화면에 급여가 뜨는 것이 싫을 때는 같은 자리에서 끕니다. iOS에도 라이브 액티비티 스위치가 따로 있는데, 이쪽 스위치는 거기서 빼기만 할 수 있습니다.

거기서 움직이는 것들은 코드가 하나도 돌지 않아도 움직입니다. 퇴근까지 남은 시간과 진행 막대는 iOS가 정해진 날짜 구간에 걸쳐 애니메이션으로 처리하므로, 앱을 마지막으로 연 지 몇 시간이 지나도 둘 다 살아 있고 정확합니다.

**금액은 그렇지 않고**, 그런 척도 하지 않습니다. 금액을 갱신하려면 앱이 앞에 떠 있거나 푸시 서버가 있어야 하는데 여기에는 둘 다 없습니다. 그래서 금액은 언제 찍힌 값인지 시각을 옆에 적어 함께 보여 줍니다. 조용히 멈춰 버린 시계는 언제 멈췄는지 말해 주는 시계보다 나쁩니다.

아일랜드를 길게 누르면 확장된 화면이 나옵니다. **탭하면 앱이 열립니다.** iOS가 탭을 그 용도로 예약해 두었고, 다른 것을 요청할 방법은 주지 않습니다.
...
- **iOS는 시뮬레이터만.** 여기 있는 어떤 것도 실제 기기용으로 서명되어 있지 않고, 무료 Apple 계정의 프로파일은 7일이면 만료되므로, 실제로 들고 다니는 폰과 워치라면 매주 다시 배포해야 합니다.
